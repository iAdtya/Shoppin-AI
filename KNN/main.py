...
class KNNImageSearchEngine:
    ...
    def build_index(self):
        """
        Build the KNN index for image embeddings.
        """
        if not self.image_embeddings:
            raise ValueError("No images have been added to the search engine.")
        embeddings_matrix = np.vstack(self.image_embeddings)
        # self.knn = NearestNeighbors(n_neighbors=5, metric='cosine').fit(embeddings_matrix)
        self.knn = NearestNeighbors(n_neighbors=min(5, len(self. image_embeddings)), metric='cosine').fit(embeddings_matrix)


    def search_similar(self, query_image_path, top_k=1):
        """
        Search for similar images using KNN given a query image.
        """
        if not self.knn:
            raise ValueError("KNN index has not been built. Call build_index() after adding images.")
        ...
        results = [(self.image_ids[idx], 1 - distances[0][i]) for i, idx in enumerate(indices[0])]
        return results
